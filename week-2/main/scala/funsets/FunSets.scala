...
object FunSets {
  ...
  def forall(s: Set, p: Int => Boolean): Boolean = {
    def iter(a: Int): Boolean = {
      if(contains(s, a) && !contains(intersect(s, p),a)) false
   ...
   */
  def map(s: Set, f: Int => Int): Set = (el:Int) => {
    def iter(old_elem: Int, new_elem: Int): Boolean = {
      if (contains(s, old_elem) && f(old_elem) == new_elem) true
      else if (old_elem == bound) false
      else iter(old_elem + 1, new_elem)
    }
    iter(-bound, el)
  }
